...
View.prototype.exercise5 = function () {
  ...
  //your code here!
  const sqr = $('.square'); // returns list of square elements
  sqr.each(function(index, el) { // run callback for each one
    const block = $(el);
    block.on("click", event => {
      alert(block.attr("data-pos"));
    })
  })
};
...
View.prototype.exercise6 = function () {
  ...
  //your code here!
  const allSqr = $('.square');
  allSqr.each(function(index, el) {
    $(el).css("background", window._randomColorString());
  })
};
...
View.prototype.exercise7 = function(){
  ...
  allSqr.each(function(index, el) {
    $(el).on("mouseover", function() {
      console.log($(el).attr("background-color"))
    })
  })
};
